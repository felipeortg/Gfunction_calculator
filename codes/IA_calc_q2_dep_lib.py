...
# Eistar = float(cval['Eb'])
 
# Efstar = float(cval['Eb'])
...
def make_int(si, sf, q2, index):
    ...
    # calculation in the initial rest frame
    Ef = (-q2 + sf + si)/(2*np.sqrt(si))
    P_i = np.array([np.sqrt(si),0,0,0])
    P_f = np.array([Ef, 0,0, np.sqrt(Ef**2-sf+0j)])
...
#CALCULATE STUFF
# q2vals = np.linspace(0,float(cval['qmax']),int(cval['evals']))
# q_shape = np.shape(q2vals)
# I_An = np.ones(q_shape) * complex(0.,0.)


# for mm, q2 in enumerate(q2vals):

#     Ei = Eistar
#     Ef = (q2 + Efstar**2 + Eistar**2)/(2*Eistar)


#     P_i = np.array([Eistar,0,0,0])
#     P_f = np.array([Ef, 0,0, np.sqrt(Ef**2-Efstar**2)])

#     I0_An = make_int(P_f, P_i, [[0],[0,0],[0,0]])
#     In_An = make_int(P_f, P_i, [[],[0,0],[0,0]])

#     I_An[mm] = (Ef+Ei)*In_An - 2*I0_An


#     print('IAcalc: ', q2, '---------', I_An[mm])
